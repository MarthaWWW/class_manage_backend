<template>
  ...
  <div class="ability">
    <div class="partA">
  <div class='search'>
  <input type="text" placeholder="请输入待查询教师真实姓名" v-model="searchName" style="background-color:white;">
  <input type="button" value="查询" class="searchButton" @click="handleSearch">
  <!-- <el-input v-model="searchName" placeholder="请输入待查询教师真实姓名" style="width:300px;"></el-input>
  ...
  <router-link to="/home/teacher/teacherEdit" style="margin-left:20px;"><el-button round @click="handleAdd" style="color:black;">增加教师</el-button></router-link>
  <router-link to="/home/teacher" style="margin-left:20px;"><el-button round style="color:black;">退出编辑</el-button></router-link>
...
export default {
  ...
  created () {
    ...
    // 如果当前操作者的身份为学院管理员，需要对数据进行过滤
    // this.originData = this.originData.filter(item => item.academy === this.managerAcademy)
    // this.total = this.originData.length
    // if (this.total > this.pageSize) { this.tableData = this.originData.slice(0, this.pageSize) } else { this.tableData = this.originData }
  },
  ...
  methods: {
    ...
    uploadFile(item) {
      ...
      console.log(msgForm)
	    ...
	    this.$axios.post('/api/mgr/import/teacher',msgForm,{headers: { 'Content-Type': 'multipart/form-data' }}).then( res => {
        if(res.data.ret===0){
          this.$message({
    ...
    handleSizeChange (val) {
      ...
      if (this.isSchool) {
        ...
        this.tableData = this.originData((this.currentPage - 1) * this.pageSize, this.currentPage * this.pageSize)
      }
    },
    // 当前页改变时触发 跳转其他页
    ...
    handleDelete (index, row) {
      console.log(index, row)
      this.$axios.delete('/api/mgr/teacher', {
      	data: {
